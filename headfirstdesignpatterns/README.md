...
1. `Context` 패턴이 적용되는 상황 
   + 반복적으로 일어날 수 있는 상황
2. `problem` 컨테스트 내에서 이루고자 하는 목적을 뜻합니다.
   + 컨텍스트 내에서 생길 수 있는 제약조건도 문제에 포함됩니다.
3. `solution` 바로 우리가 찾아내야 하는 것입니다. 누구든지 적용해서 일련의 제약조건 내에서 목적을 달성할 수 있는 일반적인 디자인을 뜻합니다.

> 어떤 컨텍스트 내에서 일련의 제약조건에 의해 영향을 받을 수 있는 문제에 봉착했다면 그 제약조건 내에서 목적을 달성하기 위한 해결책을 찾아낼 수 있는 디자인을 적용


# 유의 할점 
1. 디장인을 할 때 최대한 단순한 방법으로 문제를 해결해야한다.
2. 이 문제 대해 어떤 패턴을 적용할 수 있을지를 고민하면 안된다.
3. 패턴은 반복으로 발생하는 문제에 대한 일반적인 해결책입니다. 그러나 패턴은 `만병통치약` 아닙니다.
4. 패턴을 사용하기 전에 좀 더 쉬운 해결책을 사용하는 것을 그려해봐야합니다.
5. 시스템이 점점 복잡해진다면 처음에 기대했던 유연성이 전혀 발휘되지 않게 되는 경우 패턴을 과감하게 제거해 버리는게 낫습니다


데코레이터패턴 : 객체를 감싸서 새로운 행동을하고자 할때 
스테이트패턴: 상태를 기반으로 한 행동을 캡슐화한 다음 위임을 통해서 필요한 선택을 합니다.
이터레이터패턴 : 컬렉션이 어떤 식으로 구현되었는지 드러내진 않으면서도 컬렉션 내에 있는 모든 객체에 대해 반복 작업을 처리할 수 있게 합니다.
퍼사드 패턴 : 일련의 클래스에 대해서 간단한 인터페이스를 제공 
스트래티지패턴: 교환가능한 행동을 캡슐화하고 위임을 통해서 어떤 행동을 사용할지 결정합니다.
프록시패턴: 객체를 감싸서 그 객체에 대한 접근을 제어 
팩토리 메소드 패턴 : 생성할 구상 클래스를 서브클래스에서 결정합니다.
어댑터 패턴 : 객체를 감싸서 다른 인터페이스를 제공합니다.
